 ...
 * 
 * @author yzhou@mum.edu
 */

public class ApacheLogInMapperReducer extends Reducer<Text, Pair<Integer,Integer>, Text, FloatWritable>{
     ...
     */
    @Override
    protected void reduce(Text key, Iterable<Pair<Integer,Integer>> values,
            Context context)
            throws IOException, InterruptedException {
    
        int sum = 0;
        int count = 0;
        Iterator<Pair<Integer,Integer>> valuesIt = values.iterator();
        
        while(valuesIt.hasNext()){
            sum = sum + valuesIt.next().getT();
            count += valuesIt.next().getValue();
        }
